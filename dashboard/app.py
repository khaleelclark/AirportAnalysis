...
from content import ABOUT_MARKDOWN, CALC_MARKDOWN, HOW_TO_READ_MARKDOWN
...
with overview_tab:
    ...
    with st.expander("How To Read This Dashboard", expanded=True):
        st.markdown(HOW_TO_READ_MARKDOWN)
    ...
    def run_manual_sync_collectors() -> list[dict]:
        ...
        for label, cmd, env_overrides in commands:
            ...
            except Exception as exc:  # pragma: no cover - UI fallback guard
                results.append(
                    {
                        "label": label,
                        "ok": False,
                        "stdout": "",
                        "stderr": f"Manual sync crashed: {exc}",
                    }
                )
        return results

    def render_manual_sync_ui(key_suffix: str = "") -> None:
        st.warning(
        ...
        confirm_sync = st.checkbox(
            "I understand this triggers live API requests now.",
            key=f"confirm_manual_sync{key_suffix}",
        )
        if st.button("Run Manual Sync Now", key=f"run_manual_sync_button{key_suffix}"):
            if not confirm_sync:
    ...
    with top_controls_right:
        ...
        if popover_fn is not None:
            with st.popover("Sync Data Now (API Calls)"):
                render_manual_sync_ui()
        else:
            with st.expander("Sync Data Now (API Calls)"):
                render_manual_sync_ui("_fallback")
    st.divider()
