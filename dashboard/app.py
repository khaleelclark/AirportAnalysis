...
st.title("✈️ MCO vs DEN – Airport Delay & Traffic Dashboard")
...
with st.expander("How To Read This Dashboard", expanded=True):
    st.markdown(
        """
        - **Delay Severity Index (FAA Operational)**: 0 means no active FAA restriction. Higher values mean more severe disruption.
        - **Airline Delay Severity Index (AirLabs)**: 0-5 index from live flight delays/cancellations around each airport.
        - **Traffic Load**: Live aircraft count in airport airspace (falls back to departures + arrivals if needed).
        - **Operational Stress Score**: `(1 + Delay Severity Index) × Traffic Load` (scaled). Higher means worse passenger/operations pain.
        - **Traffic vs Delay**: Checks whether heavier aircraft traffic lines up with higher delay severity.
        - **Sync Cadence**:
          FAA Delays refresh every 10 minutes, Traffic refreshes every 10 minutes, Airline Delay refreshes every 2 hours (9 AM to 11 PM local).
        """
    )
...
def ensure_local_timestamp(value) -> pd.Timestamp:
...
if "FAA_NASSTATUS" in available_sources:
    ...
    # Fallback for older datasets that may not include FAA rows yet.
    filtered = delay_df[delay_df["airport_code"].isin(selected_airports)].copy()

if filtered.empty:
    st.warning("No rows for current filters. Adjust airport/source/time filters.")
    st.stop()

min_dt = filtered["collected_at_local"].min()
max_dt = filtered["collected_at_local"].max()

# Streamlit slider requires min < max. If only one timestamp exists, widen by 1 minute.
if min_dt == max_dt:
    min_slider = (min_dt - pd.Timedelta(minutes=1)).to_pydatetime()
    max_slider = (max_dt + pd.Timedelta(minutes=1)).to_pydatetime()
    default_value = (min_dt.to_pydatetime(), max_dt.to_pydatetime())
else:
...
start_dt = ensure_local_timestamp(date_range[0])
end_dt = ensure_local_timestamp(date_range[1])
...
if not overview_df.empty:
    ...
    top_stress = overview_df.sort_values("operational_stress_score", ascending=False).iloc[0]
